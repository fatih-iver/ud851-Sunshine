                ...
                try {
                    for(ContentValues value: values) {
                        ...
                        if(SunshineDateUtils.isDateNormalized(date)) {
                            long insertedRowUri = sqLiteDatabase.insert(WeatherContract.WeatherEntry.TABLE_NAME, null,value);
                            if(insertedRowUri != -1) {
                                succesfulInsertion++;
                            }
                        } else {
                            throw new IllegalAccessException("Date must be normalized");
                        }
